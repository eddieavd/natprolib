...
public:
        ...
        NPL_NODISCARD constexpr block_type reallocate ( block_type const & _block_, size_type _new_size_ ) noexcept
        {
                ...
                if( extend_pos < free_list_.size() )
                {
                        ...
                        if( free_list_.at( extend_pos ).len_ > _new_size_ - _block_.len_ )
                        {
                                free_list_.push_back( { static_cast< char * >(
                                                                _block_.ptr_ ) + _new_size_,
                                                        _block_.len_ + extension.len_ - _new_size_ } );
                        }
                        return reallocation;
                }
                else
